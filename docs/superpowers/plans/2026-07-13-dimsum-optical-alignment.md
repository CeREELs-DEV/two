...
**Goal:** Perfect Party의 dimsum 그림을 중앙 정렬 위치에서 왼쪽 `5px`, 아래 `8px` 이동해 다른 음식과 광학적으로 정렬한다.
...
- dimsum의 최종 불투명 경계는 `202×210+14+23`이다.
...
### Task 2: Dimsum PNG 아래 3px 추가 보정

**Files:**
- Modify: `images/DimSum.png`
- Modify: `docs/superpowers/plans/2026-07-13-dimsum-optical-alignment.md`
- Test: `tests/party-guest-images.test.mjs`

**Interfaces:**
- Consumes: Task 1에서 경계가 `202×210+14+20`으로 보정된 `images/DimSum.png`
- Produces: 가로 위치와 크기를 유지하면서 아래로 `3px` 추가 이동한 경계 `202×210+14+23`

- [x] **Step 1: 현재 자산이 새 목표 경계 검증에 실패하는지 확인**

```bash
actual="$(magick images/DimSum.png -format '%@' info:)"
test "$actual" = '202x210+14+23'
```

Expected: FAIL(exit `1`) because the current value is `202x210+14+20`.

- [x] **Step 2: 현재 PNG의 불투명 그림만 아래로 3px 이동**

```bash
magick -size 240x240 canvas:none images/DimSum.png -geometry +0+3 -composite /private/tmp/DimSum-optically-aligned-v2.png
cp /private/tmp/DimSum-optically-aligned-v2.png images/DimSum.png
```
...
Expected: `size=240x240 alpha=0..1 trim=202x210+14+23`.
...
```bash
git add images/DimSum.png docs/superpowers/plans/2026-07-13-dimsum-optical-alignment.md
git commit -m "fix: lower dimsum artwork"
```
